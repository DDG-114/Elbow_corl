...
These functions are deliberately NumPy-only. Isaac Lab integration can wrap
them later with tensor equivalents.
"""
...
def velocity_tracking_reward(actual_vel_xy: np.ndarray, cmd_vel_xy: np.ndarray, sigma: float = 0.25) -> float:
    err = np.linalg.norm(np.asarray(actual_vel_xy, dtype=np.float32)[:2] - np.asarray(cmd_vel_xy, dtype=np.float32)[:2])
    return float(np.exp(-(err * err) / max(float(sigma), 1e-6)))
...
def orientation_penalty(projected_gravity: np.ndarray) -> float:
    ...
    if gravity.size < 3:
        return 0.0
    return float(gravity[0] ** 2 + gravity[1] ** 2)


def torque_penalty(torque: np.ndarray) -> float:
    torque_arr = np.asarray(torque, dtype=np.float32)
    return float(np.mean(torque_arr * torque_arr))


def action_rate_penalty(action: np.ndarray, last_action: np.ndarray) -> float:
    action_arr = np.asarray(action, dtype=np.float32)
    last_arr = np.asarray(last_action, dtype=np.float32)
    return float(np.mean((action_arr - last_arr) ** 2))

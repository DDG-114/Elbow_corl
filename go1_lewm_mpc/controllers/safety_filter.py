...
class SafetyFilter:
    ...
    def filter_cmd(self, cmd_vel: np.ndarray) -> np.ndarray:
        ...
        limits = np.asarray(self.max_cmd_vel, dtype=np.float32).reshape(3)
        return np.clip(cmd, -limits, limits).astype(np.float32)

    def filter_action(self, action: np.ndarray) -> np.ndarray:
        return np.clip(np.asarray(action, dtype=np.float32), -float(self.max_action_abs), float(self.max_action_abs))

    def should_stop(self, obs) -> bool:
        base_pos = getattr(obs, "base_pos_w", None)
        if base_pos is not None and float(np.asarray(base_pos).reshape(-1)[2]) < self.min_base_height:
            return True

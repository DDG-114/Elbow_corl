...
def yaw_from_quat_wxyz(quat_wxyz: np.ndarray) -> float:
    ...
    if norm <= 0.0:
        raise ValueError("quat_wxyz must have positive norm")
    w, x, y, z = quat / norm
...
def body_points_to_world_xy(points_b_xy: np.ndarray, base_pos_w: np.ndarray, base_yaw: float) -> np.ndarray:
    ...
    base_xy = np.asarray(base_pos_w, dtype=np.float32).reshape(-1)[:2]
    return rotate_yaw(pts, base_yaw) + base_xy[None, :]
...
def world_points_to_body_xy(points_w_xy: np.ndarray, base_pos_w: np.ndarray, base_yaw: float) -> np.ndarray:
    ...
    base_xy = np.asarray(base_pos_w, dtype=np.float32).reshape(-1)[:2]
    return rotate_yaw(pts - base_xy[None, :], -base_yaw)
...
def body_points_to_world(points_b: np.ndarray, base_pos_w: np.ndarray, base_yaw: float) -> np.ndarray:
    ...
    base = np.asarray(base_pos_w, dtype=np.float32).reshape(-1)
    if base.size < 3:
        raise ValueError("base_pos_w must have at least three elements")
    xy = body_points_to_world_xy(pts[:, :2], base, base_yaw)
    z = pts[:, 2] + base[2]
...
def world_points_to_body(points_w: np.ndarray, base_pos_w: np.ndarray, base_yaw: float) -> np.ndarray:
    ...
    if base.size < 3:
        raise ValueError("base_pos_w must have at least three elements")
    xy = world_points_to_body_xy(pts[:, :2], base, base_yaw)
    z = pts[:, 2] - base[2]
    return np.column_stack([xy, z]).astype(np.float32)
...
def sample_points_inside_circle(center: np.ndarray, radius: float, z: float, count: int) -> np.ndarray:
    ...
    center_xy = np.asarray(center, dtype=np.float32).reshape(2)
    count = int(count)
    if count <= 0:
        raise ValueError("count must be positive")
    if radius <= 0.0 or count == 1:
        return np.array([[center_xy[0], center_xy[1], z]], dtype=np.float32)

    angles = np.linspace(0.0, 2.0 * np.pi, count, endpoint=False)
    inner_radius = float(radius) * 0.5
    points = [[center_xy[0], center_xy[1], float(z)]]
    for angle in angles[: max(0, count - 1)]:
        points.append(
            [
                center_xy[0] + inner_radius * np.cos(angle),
                center_xy[1] + inner_radius * np.sin(angle),
                float(z),
            ]
        )

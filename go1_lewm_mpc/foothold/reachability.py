...
def is_reachable_b(
    ...
    """Return whether a candidate is inside an axis-aligned reach box."""

    candidate = np.asarray(candidate_b, dtype=np.float32).reshape(3)
    nominal = np.asarray(nominal_b, dtype=np.float32).reshape(3)
    delta = np.abs(candidate - nominal)
...
def filter_reachable_b(
    ...
    mask = [is_reachable_b(candidate, nominal_b, max_dx, max_dy, max_dz) for candidate in candidates]

#!/usr/bin/env python3
"""Debug terrain-aware foothold candidate generation in mock mode."""

from __future__ import annotations

import argparse
from dataclasses import dataclass

import numpy as np

from go1_lewm_mpc.foothold.terrain_aware_generator import TerrainAwareFootholdCandidateGenerator
from go1_lewm_mpc.terrains.registry import make_terrain_generator
from go1_lewm_mpc.foothold.foothold_utils import body_points_to_world_xy
from go1_lewm_mpc.terrains.support_map import batch_query_support


@dataclass
class MockObs:
    base_pos_w: np.ndarray
    base_yaw: float
    cmd_vel: np.ndarray
    terrain_context: object


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--terrain", default="beam", choices=["flat", "beam", "stepping_stones", "mixed"])
    parser.add_argument("--swing_leg", type=int, default=0, help="0 FL, 1 FR, 2 RL, 3 RR")
    parser.add_argument("--seed", type=int, default=0)
    args = parser.parse_args()

    rng = np.random.default_rng(args.seed)
    terrain_gen = make_terrain_generator({"type": args.terrain})
    ctx = terrain_gen.query_context(np.array([0.0, 0.0, 0.28], dtype=np.float32), 0.0, rng)
    obs = MockObs(
        base_pos_w=np.array([0.0, 0.0, 0.28], dtype=np.float32),
        base_yaw=0.0,
        cmd_vel=np.array([0.2, 0.0, 0.0], dtype=np.float32),
        terrain_context=ctx,
    )
    gen = TerrainAwareFootholdCandidateGenerator()
    candidates = gen.generate(obs, args.swing_leg)
    candidates_w_xy = body_points_to_world_xy(candidates[:, :2], obs.base_pos_w, obs.base_yaw)
    support = None
    if ctx.support_map is not None:
        support = batch_query_support(ctx.support_map, candidates_w_xy, ctx.map_origin_w, ctx.map_resolution)
    print("terrain:", ctx.name)
    print("swing_leg:", args.swing_leg)
    print("candidate_count:", len(candidates))
    print("candidates_b:\n", candidates)
    if support is not None:
        print("support_values:", support)
        print("safe_count:", int(np.sum(support > 0.5)))


if __name__ == "__main__":
    main()
